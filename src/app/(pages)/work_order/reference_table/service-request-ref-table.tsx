...
const renderColumns = (
...
): ColumnDef<ServiceRequest>[] => [
  {
    ...
    header: "#",
    cell: ({row}) => {
      const isSelected = selected === row.getValue("customerRequestNo")
      return <Button
...
const ServiceRequestRefTable: React.FC<ServiceRequestRef> = ({
  ...
  const [customerRequestNo, setCustomerRequestNo] = React.useState<string | null>(null);
  const dispatch = useAppDispatch();

  useEffect(() => {
    setCustomerRequestNo(data.customerRequestNo || null)
  }, [data]);

  const handleUpdateData = (item: ServiceRequest) => {
    setCustomerRequestNo(item.requestCode)
    updateData({
      ...data,
