...
import {Options, ResolvedData, S301ServiceData, WorkOrderObj} from "@/types";
...
import {dismissAlert, showProgress, showSuccess} from "@/app/helpers/Alert";
...
const WorkOrderDetail = () => {
  ...
  const [statusCode, setStatusCode] = useState<string>("W");
  ...
  const defaultClassMenuItem = 'p-3 cursor-pointer mb-3 flex justify-center items-center';
  const workOrderRef = useRef<HTMLDivElement>(null)
  const [refType, setRefType] = useState<'ref_service_req' | 'ref_work_order' | null>(null)
  const [showWorkOrderRelation, setShowWorkOrderRelation] = useState<boolean>(false)
  const [showHistory, setShowHistory] = useState<boolean>(false)

  useEffect(() => {
    setBreadcrumb(<WorkOrderDetailBreadcrumb path={id} title={"ใบสั่งงาน"}/>)
    showProgress()
    getWorkOrderDetailById(id, statusCode).then(async (res) => {
      if (res.status === 200) {
    ...
    <div>
      <div className="flex flex-wrap justify-center items-center md:justify-between">
        ...
        <div className="flex gap-3 mb-5">
          <Button
            ...
            variant="outline"
            onClick={() => {setShowWorkOrderRelation(true)}}
          >
          ...
          <div ref={workOrderRef}>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  className="pea-button h-[44px] px-3"
                  variant="outline"
                >
              ...
              <DropdownMenuContent align="start" style={{width: workOrderRef.current?.offsetWidth + 'px'}}>
          ...
          <Button
            className="bg-[#E1D2FF] hover:bg-[#E1D2FF] rounded-full justify-center items-center h-[44px] w-[44px] p-0 cursor-pointer"
            variant="outline"
      ...
      <div className="flex flex-wrap justify-between items-center">
        ...
        <div className="flex flex-wrap items-center justify-end gap-3 w-full md:w-auto md:mt-0 mt-3">
          ...
          {
            !["B", "J", "T", "X", "Y", "Z", "M"].includes(data.workOrderStatusCode) &&
              <div className="w-full md:w-auto">
      ...
      <ModalReferenceData open={refType !== null}
                          onClose={() => setRefType(null)}
                          workOrderType={refType}
                          id={id}
      />

      <ModalRelationWorkOrder open={showWorkOrderRelation}
                              onClose={() => setShowWorkOrderRelation(false)}
                              id={id}
      />

      <HistoryModal isOpen={showHistory}
