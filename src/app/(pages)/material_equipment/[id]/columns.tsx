...
export const getColumns = (
  ...
  const onUpdateMaterial = (
    column_id_update: string,
    value: string,
    table: TypedTable,
    row: Row<MaterialEquipmentObj>
  ) => {
    let material = materialOriginalOptions.find((item) => {
      return column_id_update === 'code' ? item.code === value : item.name === value;
    })

    if(material) {
      let realValue = column_id_update === 'code' ? material.code : material.name;
      let reverseKey = column_id_update === 'code' ? 'name' : 'code';
      setTimeout(() => {
        table.options.meta?.updateData(row.index, column_id_update, realValue)
      }, 300)

      setTimeout(() => {
        let val = column_id_update === 'name' ? material.code : material.name
  ...
  return [
      ...
      cell: ({ row, table }) => {
        return row.original.isUpdate ? (
          <EditableSelectCell
            ...
            onUpdate={(value: string) => onUpdateMaterial('code', value, table as TypedTable, row)}
          />
        ) : (
      ...
      cell: ({ row, table }) => {
        return row.original.isUpdate ? (
          <EditableSelectCell
            ...
            onUpdate={(value: string) => onUpdateMaterial('name', value, table as TypedTable, row)}
    ...
    },
    {
      ...
      maxSize: 25,
      cell: ({ row }) => {
        return materialUnitMap.get(row.getValue('code')) || '-';
      }
    },
    {
