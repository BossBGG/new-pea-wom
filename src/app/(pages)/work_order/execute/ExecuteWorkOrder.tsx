...
export const ExecuteWorkOrder: React.FC<ExecuteWorkOrderProps> = ({
  ...
  const defaultSignature =  data?.execution?.recorderSignature || useAppSelector(state => state.user?.signatureImageUrl) || ""
      ...
      {screenSize === "mobile" ? (
        ...
        <>
          ...
          <RecordKeeper
            options={{
              isReadOnly: disabled,
              initialRecordKeeperSignature: defaultSignature
            }}
            data={data}
            setData={setData}
          />
