...
const ModalNewWorkOrder = ({
  ...
  const submit = async () => {
    ...
    if(res.status === 201) {
      ...
      if(service) {
        // router.push(`/work_order/${service}?id=${res.data.data?.id}&workOrderNo=${res.data.data?.workOrderNo}`)
        router.push(`/work_order/create_or_update?id=${res.data.data?.id}&requestCode=${service}&workOrderNo=${res.data.data?.workOrderNo}`)
      }else {
    ...
    >
     <div className="w-full">
       <InputRadio label="เลือกประเภทการสร้างใบงาน"
                   ...
                   value={data.workOrderType || workOrderTypeOptions[0].value}
