  ...
  useEffect(() => {
    if (data.participants && data.participants.length > 0) {
      const mapped = data.participants.map((p: any) => {
        if (p.userType !== undefined) {
          return {
            ...p,
            isUpdate: p.isUpdate !== undefined ? p.isUpdate : false,
          };
        }
        return {
          ...itemPerson,
          username: p.username || "",
          isUpdate: p.username ? false : true,
        };
      });
    ...
    data.participants?.map((p) => p.username).join(","),
  ]);

  const handleUpdateData = (updatedPersons: Assignee[]) => {
    setInvolvedPersons(updatedPersons);

    const mappedParticipants = updatedPersons
      .filter((person) => person.username && person.username.trim() !== "")
      .map((person) => ({
    ...
    const newData = {
      ...data,
      participants: mappedParticipants as any,
    };
