'use client';
import {Electrical, Options, RequestServiceItem} from "@/types";
...
interface ListDataContentProps {
  realData: RequestServiceItem[],
  pageData: RequestServiceItem[],
  onUpdateData: (data: RequestServiceItem[]) => void,
...
const ListDataContent = ({
  ...
  const handleUpdateData = (key: string, value: string | number | boolean | undefined, index: number) => {
    ...
    const newData = realData.map((item: RequestServiceItem, idx) => {
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <div className="font-medium text-sm mb-2 text-gray-800">
                      {index + 1}. {equipmentNameOptions.find((opt) => opt.value === item.item_id)?.label || item.item_id}
                    </div>

                    {/* จำนวนหน่วย */}
                    <div className="text-sm text-gray-600">
                      <span>จำนวนหน่วย : </span>
                      <span className="font-medium text-sm text-black">{item.quantity || 2}</span>
                    </div>
                  </div>
                  ...
                  {showActionButtons && !isReadOnly && (
                    <div className="flex space-x-2 ml-4">
                      ...
                      <button
                        ...
                        onClick={() => deleteData(item.item_id || "", index)}>
