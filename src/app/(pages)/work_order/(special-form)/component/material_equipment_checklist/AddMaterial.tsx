...
const AddMaterial = ({
  ...
  const isItemAlreadySelected = (item: MaterialEquipmentObj) => {
    return existingMaterials.some(existing => 
      existing.code === item.code && existing.name === item.name
    );
  };

  const loadMaterialSets = async (materialUuid: string, searchName: string = "") => {
    if (!materialUuid) return;

    if (!office) {
      console.error("Office code is required");
    ...
    try {
        ...
        if (selectedSet) {
          const equipmentList: MaterialEquipmentObj[] =
            selectedSet.materialAndEquipment.map((item) => {
              const existingItem = existingMaterials.find(existing => 
                existing.code === item.code && existing.name === item.name
              );
      ...
      console.error("Error loading material sets:", error);
      setFilteredEquipmentList([]);
    } finally {
      setLoading(false);
    }
  ...
  const loadMaterialMaster = async (search: string) => {
    ...
    try {
      ...
      if (response.status === 200 && response.data.data) {
        ...
        const equipmentList: MaterialEquipmentObj[] = materials.map((item) => {
          ...
          return {
            ...
            code: item.code,
            name: item.name,
            quantity: editMode && editingItem ? editingItem.quantity : (existingItem ? existingItem.quantity : 1),
            unit: item.unit,
            price: item.price,
            availableStock: 0,
            isActive: true,
      ...
      console.error("Error loading material master:", error);
      setFilteredEquipmentList([]);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    setCurrentPage(0);
  }, [material, active, searchText, office, selectedSetName]);
  ...
  return (
    <Modal
      classContent="!h-[75%] flex flex-col justify-start"
    ...
    >
      ...
      <div className="overflow-x-hidden h-[100%] max-h-[100%]">
        ...
        {loading ? (
          <div className="text-center text-gray-500 p-4">กำลังโหลดข้อมูล...</div>
        ) : paginatedItems.length > 0 ? (
          <>
            {!editMode && (
                  ...
                  >
                    <div className="flex flex-row justify-between w-full">
                      <div className="font-medium w-[65%]">
                        {actualIndex + 1}. {item.code} - {item.name}
                      </div>
