...
import {
  executeWorkOrder, getServiceRequestDetail,
...
const CreateOrUpdateWorkOrder = () => {
  ...
  const materialEquipmentRef = useRef<{ loadInitialMaterials: () => void }>(
    null
  );
  ...
  const fetchAllData = () => {
    ...
    Promise.all(promiseValues)
      .then(async (results) => {
        ...
        if (isEdit === "true" || isView === "true" || isExecute === "true") {
          ...
          if (customerRequest?.workOrderParentId) {
            ...
            if(customerRequest?.customerRequestNo) {
              ...
              const resCustReq = await getServiceRequestDetail(customerRequest.customerRequestNo)

              if(resCustReq.status === 200) {
                const customerItem: CustomerRequest[] = resCustReq.data?.data?.data || [{} as CustomerRequest]
                if(customerItem.length > 0) {
                  newData = {
                    ...newData,
                    request_no: customerRequest?.customerRequestNo || "",
                    customerName: customerItem[0]?.customerName || "",
                    customerMobileNo: customerItem[0].customerMobileNo || "",
                    customerAddress: customerItem[0].customerAddress || "",
                    customerEmail: customerItem[0].customerEmail || "",
                    customerBp: customerItem[0].customerBp || "",
                    customerCa: customerItem[0].customerCa || "",
                    customerLatitude: customerItem[0].customerLatitude || "",
                    customerLongitude: customerItem[0].customerLongitude || "",
                  }
                }
  ...
  const fetchWorkOrderDetail = async (
    ...
    getWorkOrderDetailById(workOrderId, statusCode).then(async (res) => {
      if (res.status === 200) {
        ...
        if (requestCode === "s318") {
          const serviceSpecS318 = items.serviceSpecificData as S318ServiceData;
      ...
      showSuccess().then(async (res) => {
        if (status && status === "M") {
          ...
          const params = new URLSearchParams({
            ...
            workOrderNo,
            requestCode,
            isExecute: item.workOrderStatusCode === "O" ? "true" : "false",
            isEdit: item.workOrderStatusCode === "W" ? "true" : "false",
            statusCode: item.workOrderStatusCode
          });

          router.push(`/work_order/create_or_update?${params.toString()}`);
          fetchAllData();

          if (currentStep === 2 && materialEquipmentRef.current) {
            materialEquipmentRef.current.loadInitialMaterials();
          }
        }
      });
    } else {
