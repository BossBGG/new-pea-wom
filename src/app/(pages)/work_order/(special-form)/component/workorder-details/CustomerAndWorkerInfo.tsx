...
import {Customer, Options, WorkerObj, WorkOrderObj} from "@/types";
      ...
      >
        <div className="flex flex-wrap items-stretch">
                              ...
                              >
                                ...
                                >
                                  <div className="flex-1">
                                      <span className="font-medium text-gray-900">
                                        {index + 1}. {worker.username} - {" "}
                                        {workerOptions.find((wk) => wk.value == worker.username)?.label || "ไม่ระบุชื่อ"}
                                      </span>
                                  </div>
                                ...
                                {expandedWorkers[index] && (
                                  <div className="p-3 bg-white space-y-2">
                                    <CustomerInform title="กลุ่มผู้ปฏิบัติงาน" value={worker.userType || ""}/>
                                    ...
                                    <CustomerInform title="ชั่วโมง/งาน"
                                                    value={`${worker.workHours} ${worker.workUnit || "H"}`}
                                    />
                                  </div>
                                )}
                              </div>
          ...
          <div className="w-full mt-3 xl:mt-0 xl:w-1/3 px-2">
            <Map
              latitude={data.customerLatitude ? parseFloat(data.customerLatitude) as number : 0}
              longitude={data.customerLongitude ? parseFloat(data.customerLongitude) as number : 0}
              options={{
                showEditable: false,
              }}
            />
