...
import {UploadedFile, WorkOrderObj} from "@/types";
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validFiles = Array.from(files).filter((file) => {
      ...
      if (sanitizedName !== file.name) {
        showError(`ชื่อไฟล์ ${file.name} มีอักขระที่ไม่อนุญาต`);
        return false;
      }
    ...
    validFiles.forEach(async (file, index) => {
      try {
        ...
        const uploadFiles = [...uploadedFiles, uploadedFile]
        let ids: number[] = []
        uploadFiles.map((f) => {
          ids.push(f.id)
        })
        ...
        let newData = {...data, attachments: ids}
        setData(newData)
        processedCount++;
      } catch (error) {
