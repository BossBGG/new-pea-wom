...
const AddMaterial = ({
  ...
  const loadMaterialSets = async (materialUuid: string) => {
    if (!materialUuid) return;

    setLoading(true);
    try {
      const response = await getMaterialSets("");
        ...
        if (selectedSet) {
          ...
          if (editMode && editingItem) {
            const matchingItem = equipmentList.find(
              (item) =>
                item.code === editingItem.code && item.name === editingItem.name
            );
            if (matchingItem) {
    ...
    try {
      ...
      if (response.status === 200 && response.data.data) {
        ...
        const equipmentList: MaterialEquipmentObj[] = materials.map((item) => ({
          ...
          code: item.code,
          name: item.name,
          quantity: editMode && editingItem ? editingItem.quantity : 1,
          unit: item.unit,
          price: item.price,
          isActive: true,
          isUpdate: false,
  ...
  const handleSubmit = () => {
    ...
    if (selectedEquipments.length > 0) {
      onAddMaterial?.(selectedEquipments);
    }
