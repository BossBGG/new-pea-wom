  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if(materials.length > 0) {
        ...
        materials.map((mat) => {
          let inOption = response_options.original.find((ori) => ori.id === mat.id)
          if(!inOption) {
            material_original = [...material_original, mat]
            material_code = [...material_code, {value: mat.code, label: mat.code, data: mat}]
            material_name = [...material_name, {value: mat.name, label: mat.name, data: mat}]
          }
        })
  ...
  return (
    <div className="border-1 border-[#E1D2FF] rounded-[20px] p-4">
      <div
        className={
          screenSize !== "desktop"
            ? "flex flex-wrap md:flex-nowrap items-end w-full mb-3"
            : "w-full"
        }
      >
        <InputText
