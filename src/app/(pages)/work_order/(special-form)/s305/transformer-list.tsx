...
import {Options, Transformer, TransformerMaterialEquipmentObj, WorkOrderObj} from "@/types";
...
const TransformerList = ({
  ...
  const [editData, setEditData] = React.useState<Transformer | null>(null);
  ...
  useEffect(() => {
    if (typeof data.requestServiceDetail === 'object') {
      let transformerData = data.requestServiceDetail?.items || [];
      console.log('transformerData >>>> ', transformerData);
      setTransformers(transformerData as Transformer[])
    }
  }, [data.requestServiceDetail]);
  ...
  const handleUpdateData = (transformer: Transformer[]) => {
  ...
  const handleAddTransformer = (transformer: Transformer) => {
    let newTransformers: Transformer[] = transformers;
    if(updateIndex > -1) {
    ...
    if (screenSize === "desktop") {
      return (
        <DataTableEditor
          ...
          LabelAddRow={showAddButton && screenSize === "desktop" ? "เพิ่มหม้อแปลง" : undefined}
