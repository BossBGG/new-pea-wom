    ...
    },
    {
      ...
      cell: ({row}) => {
        ...
        ServiceTypeOptions.map((ser) => {
          let findService = ser.subOptions?.find((sub) => sub.data.name === row.original.serviceType)
          if(findService) service = findService
        })

        if(service && service.data) {
          const requestCode = service.data.requestCode.toLowerCase();
          return <div className="flex items-center">
            ...
            {/*<Link href={`/work_order/${requestCode}?id=${row.original.id}&workOrderNo=${row.original.workOrderNo}&isEdit=true`}
                  className="bg-[#FDE5B6] rounded-sm px-2 py-1">
              <FontAwesomeIcon icon={faPencil} color="#F9AC12"/>
            </Link>*/}

            <Link href={`/work_order/create_or_update?id=${row.original.id}&workOrderNo=${row.original.workOrderNo}&requestCode=${requestCode}&isEdit=true`}
                  className="bg-[#FDE5B6] rounded-sm px-2 py-1">
              <FontAwesomeIcon icon={faPencil} color="#F9AC12"/>
            </Link>
          </div>
