...
import {Options, RequestServiceItem} from "@/types";
...
interface ModalEquipmentsProps {
  ...
  onUpdateData:(d: RequestServiceItem[]) => void,
  requestCode: string
}
...
const ModalEquipments = ({
  ...
  const [equipmentSelected, setEquipmentSelected] = useState<RequestServiceItem[]>(data);
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      let newData = serviceEquipmentOptions.map((sv) => (
        {
          item_id: sv.value as string,
          quantity: getQuantity(sv),
          isUpdate: false,
          isEdited: false
        }
      ))
  ...
  const handleCheck = (checked: boolean, item: Options) => {
    if (checked) {
      ...
      const newData = equipmentSelected.filter((eq) => eq.item_id !== item.value)
  ...
  const handleQuantityChange = (id: string, newQuantity: string) => {
    const newData = equipmentSelected.map((d) =>
      d.item_id === id ? { ...d, quantity: newQuantity } : d
    );
  ...
  const getCheckState = (item: Options) => {
    if(equipmentSelected.length > 0) {
      if(equipmentSelected.find((d) => d.item_id === item.value)) {
        return true
      }
      return false;
    }
  ...
  const getQuantity = (item: Options) => {
    return equipmentSelected.find((d) => d.item_id === item.value)?.quantity || 0
  }
