...
import {mapEventOptions, mapWorkCenterOptions} from "@/app/(pages)/work_order/create_or_update/mapOptions";
...
const ServiceSatisfactionDetailPage = () => {
  ...
  useEffect(() => {
    setBreadcrumb(<ServiceSatisfactionDetailBreadcrumb title={"รายละเอียดการประเมินความพึงพอใจ"}/>)
      ...
      if(res.status === 200) {
        ...
        Promise.all(promiseValues)
          .then(async (results) => {
            ...
            const {respEventOptions, resMainWorkCenter, resWorkerOptions} = resolvedData;
            setEventOptions(respEventOptions || [])
            setMainWorkCenterOptions(resMainWorkCenter || [])

            if(items.workers?.length > 0) {
              let assignees = items.workers;
              const newEventOpts = await mapEventOptions(assignees, eventOptions || [], "activity")
              setEventOptions(newEventOpts)
              const newWorkCenterOptions = await mapWorkCenterOptions(assignees, resMainWorkCenter || [], "mainWorkCenter")
    ...
    <div className="px-6">
        ...
        <CardContent className="p-5">
          <div className="flex flex-wrap">
            <div className="w-full md:w-1/2 px-2 md:border-r-1 md:border-r-[#AAAAAA]">
              <InfoRow title="เลขที่คำร้อง" value={data.workOrder?.workOrderNumber || "-"}/>
              <InfoRow title="เลขที่คำร้องจาก SAP" value={data.serviceRequest?.sapRequestNumber || "-"}/>
              <InfoRow title="ประเภทใบสั่งงาน" value={data.serviceRequest?.serviceTypeName || "-"}/>
            ...
            <div className="w-full md:w-1/2 px-2">
              <InfoRow title="ประเภทงานบริการ" value={data.serviceRequest?.serviceType || "-"}/>
              <InfoRow title="ช่องทางรับคำร้อง" value={data.serviceRequest?.channel || "-"}/>
              <InfoRow title="วันที่รับคำร้อง" value={data.serviceRequest?.receivedDate ? formatJSDateTH(new Date(data.serviceRequest?.receivedDate), "d MMMM yyyy") : "-"}/>
            </div>
          </div>
        </CardContent>
      </Card>

      <CardCollapse title={"ข้อมูลผู้ขอคำร้อง / ตำแหน่ง /  ที่อยู่สำหรับสำรวจพื้นที่"}>
        <div className="flex flex-wrap items-stretch">
          ...
          <div className="w-full lg:w-1/3">
            <Card className="bg-white shadow-sm h-full">
              <CardContent className="p-6">
                ...
                <InfoRow title="ชื่อลูกค้า" value={data.customer?.name || "-"}/>
                ...
                <div className="flex flex-row justify-between mb-3">
                  ...
                  <div className="flex items-center">
                    {
                      data.customer?.phone ?
                        <a href={`tel:${data.customer.phone}`} className="flex items-center">
                          <FontAwesomeIcon icon={faPhone} color="#03A9F4" className="mr-2"/>
                          <span className="text-[#03A9F4] underline">{data.customer.phone}</span>
                        </a>
                        : <span>-</span>
                    }
                  </div>
                ...
                <InfoRow title="อีเมลติดต่อ" value={"Sujinda_yuda@email.co.th"}/>
                <InfoRow title="BP" value={data.customer?.bp || "-"}/>
                <InfoRow title="CA" value={data.customer?.ca || "-"}/>

                <hr className="my-4"/>

                <InfoRow title="ที่อยู่ขอรับบริการ" value={"ไฟชั่วคราว ต.หนองขนาน อ.เมืองเพชรบุรี จ.เพชรบุรี 76000"}/>
                <InfoRow title="Latitude" value={data.customer?.latitude?.toString()}/>
                <InfoRow title="Longitude" value={data.customer?.longitude?.toString()}/>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="w-full lg:w-1/3 lg:px-3 lg:my-0 my-3">
            <Card className="bg-white shadow-sm h-full">
              <CardContent className="p-6">
                <div className="bg-[#E1D2FF] text-center py-3 rounded-lg mb-4">
                  <div className="font-medium text-[#160C26]">รายชื่อผู้ปฏิบัติงาน</div>
                </div>
                              ...
                              >
                                ...
                                >
                                  <div className="flex-1">
                                      <span className="font-medium text-gray-900">
                                        {index + 1}. {worker.employeeId ? `${worker.employeeId} - ` : ""}
                                ...
                                {expandedWorkers[index] && (
                                  <div className="p-3 bg-[#F2F2F2] space-y-2">
                                    <InfoRow title="กลุ่มผู้ปฏิบัติงาน" value={ groupWorkerOptions.find((group) => group.value === worker.group)?.label  || ""}/>
                                    <InfoRow title="กิจกรรม"
                                             value={eventOptions.find((ev) => ev.value == worker.activity)?.label || "-"}
                                    />
                                    <InfoRow title="ศูนย์งานหลัก"
                                             value={workCenterOptions.find((ev) => ev.value == worker.mainWorkCenter)?.label || "-"}
                                    />
                                    <InfoRow title="ชั่วโมง/งาน"
                  ...
                  <div>
                    <div className="flex justify-between pb-1">
                      ...
                      <span className="font-medium">
                            {
                              data.appointment ? data.appointment.appointmentDate : '-'
                            }
                          </span>
                    </div>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{data.appointment ? data.appointment.appointmentTime : "-"}</span>
                    </div>
                  </div>
                </div>
              </CardContent>
        ...
        <CardCollapse title={"ความพึงพอใจต่อการให้บริการ"}>
          <CardContent>
            <div className="flex items-center mb-3">
              ...
              <div className="mx-2"> {data.satisfaction?.overallRating || 0}/5</div>
              <div className="-mt-[5px]">
                <StarRating score={data.satisfaction?.overallRating || 0} showScore={false} />
              </div>
            </div>
      ...
      <div className="flex flex-wrap justify-between items-center mt-6 w-full">
        ...
        <div className="flex gap-3 md:w-auto w-full mt-4 md:mt-0">
          <Button className="rounded-full cursor-pointer text-white bg-[#671FAB] hover:bg-[#671FAB] w-full"
                  onClick={() => router.push(`/work_order/${id}`)}
          >
            ผลปฏิบัติงาน
          </Button>
        </div>
